        ...
        >
          {museum.museum.exhibitionsList.map((exhibition, index) => (
            <TouchableOpacity
              ...
              style={
                index === museum.museum.exhibitionsList.length - 1
                  ? { paddingBottom: 700 }
                  : {}
              }
            >
              <View style={[styles.exhibition]}>
                <View style={[styles.exhibitionHeader]}>
                  <ZailaText style={[styles.exhibitionName]} weight="semiBold">
                    {exhibition.name.substring(0, 15) +
                      (exhibition.name.length > 15 ? "..." : "")}
                  </ZailaText>
                </View>
                <View style={[styles.exhibitionInfo]}>
                  <View style={[styles.exhibitionImgWrapper]}>
                    {console.log(
                      "Exhibition Category",
                      exhibition.exhibition_category
                    )}
                    <Image
                      ...
                      <Image
                        source={
                          categories.find(
                            (category) =>
                              category.name ===
                              exhibition.exhibition_category.categoryName
                          ).img
                        }
                      />
                    </View>
                  ...
                  <View style={[styles.exhibitionDescWrapper]}>
                    <ZailaText style={[styles.exhibitionDate]} weight="light">
                      {formatDate(exhibition.startDate).toUpperCase()} -{" "}
                      {formatDate(exhibition.endDate.toUpperCase())}
                    </ZailaText>
                    <ZailaText
